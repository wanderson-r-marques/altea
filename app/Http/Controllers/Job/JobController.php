...
class JobController extends Controller
{
    ...
    public function applyJob(Request $request, $job_slug)
    {
        ...
        if ((bool)$user->is_active === false) {
            flash(__('Para ativar sua conta <a class="text-warning" href="'.route('my.profile').'#cvs"><b>ENVIE O LAUDO MÉDICO</b></a>. Se já enviou, aguarde a análise de nossa equipe.'))->error();
            return \Redirect::route('job.detail', $job_slug);
            exit;
        }
        ...
        if ((bool) config('jobseeker.is_jobseeker_package_active')) {
            if (
                ...
                flash(__('Por favor, inscreva-se no pacote primeiro'))->error();
                return \Redirect::route('home');
                exit;
            }
        }
        if ($user->isAppliedOnJob($job->id)) {
            flash(__('Você já se inscreveu para este trabalho'))->success();
            return \Redirect::route('job.detail', $job_slug);
            exit;
        }
    ...
    public function postApplyJob(ApplyJobFormRequest $request, $job_slug)
    {
        ...
        // VERIFICAR ISSO AQUI QUE COMENTEI !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        //event(new JobApplied($job, $jobApply));

        flash(__('Você se candidatou a este emprego com sucesso'))->success();
        return \Redirect::route('job.detail', $job_slug);
    }
