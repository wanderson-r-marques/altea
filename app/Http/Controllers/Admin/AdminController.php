...
class AdminController extends Controller
{
    ...
    public function storeAdminUser(AdminFormRequest $request)
    {
        ...
        /*         * ******************** */
        flash('Novo usuário administrador foi criado!')->success();
    ...
    public function updateAdminUser($id, AdminFormRequest $request)
    {
        ...
        $user->role_id = $request->role_id;
        $user->save();
        flash('O usuário administrador foi atualizado!')->success();
        return \Redirect::route('edit.admin.user', array($user->id));
    }
    ...
    public function fetchAdminUsersData()
    {
        ...
        return Datatables::of($users)
                        ->addColumn('action', function ($user) {
                            return '<a href="' . route('edit.admin.user', ['id' => $user->id]) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i> Editar</a><a href="javascript:void(0);" onclick="delete_user(' . $user->id . ');" class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-remove"></i> Deletar</a>';
                        })
