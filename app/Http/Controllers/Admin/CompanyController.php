...
class CompanyController extends Controller
{
    ...
    public function storeCompany(CompanyFormRequest $request)
    {
        ...
        /*         * ************************************ */
        flash('Empresa foi adicionada!')->success();
    ...
    public function updateCompany($id, CompanyFormRequest $request)
    {
        ...
        /*         * ************************************ */
        flash('Empresa foi atualizada!')->success();
        return \Redirect::route('edit.company', array($company->id));
    }
    ...
    public function fetchCompaniesData(Request $request)
    {
        ...
        return Datatables::of($companies)
                        ...
                        ->addColumn('action', function ($companies) {
                            ...
                            /*                             * ************************* */
                            $featuredTxt = 'Verificado';
                            $featuredHref = 'makeFeatured(' . $companies->id . ');';
                            $featuredIcon = 'square-o';
                            if ((int) $companies->is_featured == 1) {
                                $featuredTxt = 'Não verificado';
                                $featuredHref = 'makeNotFeatured(' . $companies->id . ');';
                                $featuredIcon = 'check-square-o';
                            }
                            return '
				<div class="btn-group">
					<button class="btn btn-warning dropdown-toggle" data-toggle="dropdown" aria-expanded="false">Ação
						<i class="fa fa-angle-down"></i>
					</button>
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('list.jobs', ['company_id' => $companies->id]) . '" target="_blank"><i class="fa fa-list" aria-hidden="true"></i>Anúncios</a>
						</li>
						...
						<li>
							<a href="javascript:void(0);" onclick="deleteCompany(' . $companies->id . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Deletar</a>
						</li>
