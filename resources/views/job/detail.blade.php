    ...
    <div class="container"> 
        ...
        <div class="job-header">
            <div class="jobinfo">
                        ...
                        <div class="salary">{{$job->getSalaryPeriod('salary_period')}}: <strong>{{($job->salary_currency == 'BRL') ? 'R$' : $job->salary_currency}} {{  number_format($job->salary_from, 2, ',', '.') }} até {{($job->salary_currency == 'BRL') ? 'R$' : $job->salary_currency}} {{  number_format($job->salary_to, 2, ',', '.')}}</strong></div>
                        @endif
						@endif
            ...
            <div class="col-lg-5"> 
				...
				<div class="companyoverview">
					...
					<p>{{\Illuminate\Support\Str::limit(strip_tags($company->description), 250, '...')}} <a href="{{route('company.detail',$company->slug)}}">{{__('Read More')}}</a></p>
					</div>
                        </div>
