        ...
        <div class="row"> @foreach($packages as $package)
            <div class="col-md-4 col-sm-6 col-xs-12">
                <ul class="boxes">
                    ...
                    <li>
                        <div class="main-plan">
                            <div class="plan-price1-1">{{ ($siteSetting->default_currency_code == 'BRL') ? 'R$' : '$siteSetting->default_currency_code' }}</div>
                            <div class="plan-price1-2"> {{number_format($package->package_price,2,",",".")}}</div>
                    ...
                    @if((bool)$siteSetting->is_payu_active)
                       <li class="order payu"><a href="{{route('payu.order.package', ['package_id='.$package->id, 'type=upgrade'])}}"><i class="fa fa-cc-paypal" aria-hidden="true"></i> {{__('pay with PayU')}}</a></li>
                    @endif
