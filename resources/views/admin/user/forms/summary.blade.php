...
<form class="form" id="add_edit_profile_summary" method="POST" action="{{ route('update.profile.summary', [$user->id]) }}">{{ csrf_field() }}
    <div class="form-body">
        ...
        <div class="form-group" id="div_summary">
            <label for="summary" class="bold">{{__('Profile Summary')}}</label>
            <textarea name="summary" class="form-control" id="summary" placeholder="{{__('Profile Summary')}}">{{ old('summary', (isset($user))? $user->getProfileSummary('summary'):'') }}</textarea>
    ...
    function submitProfileSummaryForm() {
        ...
        $.ajax({
            ...
            success: function (json) {
                $("#success_msg").html('<span class="text text-success">Resumo atualizado com sucesso!</span>');
            },
            error: function (json) {
                if (json.status === 422) {
